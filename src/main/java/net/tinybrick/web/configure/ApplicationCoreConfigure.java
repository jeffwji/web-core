...
public class ApplicationCoreConfigure extends WebMvcConfigurerAdapter {
     ...
     */
    //@Value("#{'${web.static.classpath_resource:/images=/images,/css=/css,/static=/static,/public=/public,=/js}'.split(',')}")
    //List<String> classPathResources;
    @Value("#{${web.static.classpath_resource:{images:'/images',css:'/css',static:'/static',public:'/public',js:'/js'}}}")
    private Map<String, String> resourceMap;

    @Bean
    public WebResources getStaticResources() {
        WebResources webResource = new WebResources();
        webResource.setStaticResources(resourceMap);
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        Map<String, String> resourceMap = getStaticResources().getStaticResources();
        Set<String> paths = resourceMap.keySet();
        for (String path : paths) {
            registry.addResourceHandler("/" + path + "/**").addResourceLocations("classpath:" + resourceMap.get(path) + "/");
        }
    }
    ...
    @Value("${email.stmp.enabled:true}")
    boolean mail_stmp_enabled;
    @Value("${email.stmp.auth:true}")
    boolean mail_stmp_auth;
    @Value("${email.stmp.from:nobody}")
    String defaultSender;
    @Value("${email.stmp.port:25}")
    int mail_stmp_port;
    @Value("${email.stmp.host:mail.htche.com}")
    String mail_stmp_host;
    @Value("${email.stmp.user:}")
    String mail_stmp_user;
    @Value("${email.stmp.pass:}")
    String mail_stmp_pass;
    ...
    public static class GeneralControllerExceptionAdvice {
        ...
        protected @Autowired
        NotificationDelivererChain notificationDelivererChain;

        @ExceptionHandler(value = {Throwable.class})
        public @ResponseBody
        ResponseEntity<Object> onGeneralError(final Throwable t,
